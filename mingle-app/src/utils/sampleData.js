// export const postData = {
//     fullName: "Kal Wardin",
//     username: "kalwardin07",
//     title: "Embracing the synth magic tonight. Retrowave is my muse.🚀",
//     type: "image",
//     description: "Let's talk about dopamine and its effects on motivation and drive. Wake up look at the soft gleaming sunlight",
//     mediafiles: [
//         "https://png.pngtree.com/thumb_back/fw800/background/20240310/pngtree-aesthetic-mountain-synthwave-retrowave-wallpaper-with-a-cool-and-vibrant-neon-image_15639256.jpg",
//         "https://images.unsplash.com/photo-1516802273409-68526ee1bdd6",
//         "https://wallpapercave.com/wp/wp4403503.jpg",
//         "https://images.unsplash.com/photo-1567306301408-9b74779a11af",
//     ],
//     tags: ["#uplifting", "#motivation"],
//     createdAt: new Date(2024, 10, 23, 16, 47, 22),
//     avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
//     liked: true,
//     likesCount: 200,
//     commentsCount: 300,
//     saved: true
// }

export const postData = {
    _id:"123asd",
    fullName: "Kal Wardin",
    username: "kalwardin07",
    title: "Embracing the synth magic tonight. Retrowave is my muse.🚀",
    type: "article",
    article: "When we keep externals in the proper perspective, we gain a steadiness (eustatheia) that helps us along life’s way.\n\nIn the real world, we all have things we need and duties and obligations that arise from our family, relationships, and vocations. Zeno was the first to divide these externals into what he called “preferred and dispreferred indifferents.” \n\nWhat he meant is that while they don’t have intrinsic moral value, they form a kind of second class of value that is an important part of our lives—things like health and wealth are to be preferred over sickness and poverty, and if we are lucky enough to have them they can be a benefit to us and others as we pursue a virtuous life.\n😊😊😊",
    tags: ["#uplifting", "#motivation"],
    createdAt: new Date(2024, 10, 23, 16, 47, 22),
    avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
    liked: false,
    likesCount: 200,
    commentsCount: 300,
    saved: false
}

export const myProfile = {
    fullName: "Kal Wardin",
    username: "kalwardin07",
    email: "kalwardin@email.com",
    avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
    headline: "Early Bird | Synthwave enthusiast | Programmer 🚀🌇💻",
    biography: "Immersed in the ocean of programming trying to solve real world problems by putting those brain cells to work.",
    publicUrls: [
        "https://github.com",
        "https://leetcode.com"
    ],
    createdAt: new Date(2024, 10, 23, 16, 47, 22),
    followers: 230,
    following: 20,
    follow: false
}

export const myProfile1 = {
    firstName: "Kal",
    lastName: "Wardin",
    username: "kalwardin07",
    email: "kalwardin@email.com",
    avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
    headline: "Early Bird | Synthwave enthusiast | Programmer 🚀🌇💻",
    biography: "Immersed in the ocean of programming trying to solve real world problems by putting those brain cells to work.",
    publicUrls: [
        "https://github.com",
        "https://leetcode.com"
    ],
    createdAt: new Date(2024, 10, 23, 16, 47, 22),
    followers: 230,
    following: 20,
    follow: false
}

export const otherUserProfile = {
    fullName: "John Doe",
    username: "johndoe09",
    email: "johndoe09@email.com",
    avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-wby69l6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=61341c3ce447f8356da3146c1903395fc43d28dc",
    headline: "Coffee Lover ☕| Blockchain Enthusiast | Explorer",
    biography: "Immersed in the ocean of blockchain trying to solve real world problems by putting those brain cells to work.",
    publicUrls: [
        "https://github.com",
        "https://leetcode.com"
    ],
    createdAt: new Date(2024, 10, 23, 16, 47, 22),
    followers: 47000,
    following: 200,
    connected: true
}

export const sampleTrendingPosts = {
    posts: [
        {
            fullName: "Steve Rogers",
            username: "steverogers04",
            id: "123312",
            avatar: "https://img.freepik.com/premium-photo/cartoon-character-with-blue-hair-glasses-with-word-winter-it_784625-9610.jpg?semt=ais_hybrid",
            title: "The limitations of AI are fascinating",
            likes: 230,
            comments: 400
        },
        {
            fullName: "Steve Rogers",
            username: "steverogers04",
            id: "3213123",
            avatar: "https://img.freepik.com/premium-photo/cartoon-character-with-blue-hair-glasses-with-word-winter-it_784625-9610.jpg?semt=ais_hybrid",
            title: "The limitations of AI are fascinating",
            likes: 230,
            comments: 400
        }
    ],
    tags: [
        "uplifting",
        "motivation",
        "economy"
    ]
}

export const postList = [
    {
        _id: "123444342",
        fullName: "Kal Wardin",
        username: "kalwardin07",
        title: "Embracing the synth magic tonight. Retrowave is my muse.🚀",
        type: "image",
        description: "Let's talk about dopamine and its effects on motivation and drive. Wake up look at the soft gleaming sunlight",
        mediafiles: [
            "https://images.unsplash.com/photo-1516802273409-68526ee1bdd6",
            "https://wallpapercave.com/wp/wp4403503.jpg",
            "https://png.pngtree.com/thumb_back/fw800/background/20240310/pngtree-aesthetic-mountain-synthwave-retrowave-wallpaper-with-a-cool-and-vibrant-neon-image_15639256.jpg",
            "https://images.unsplash.com/photo-1567306301408-9b74779a11af",
        ],
        tags: ["#uplifting", "#motivation"],
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
        avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
        liked: true,
        likesCount: 200,
        commentsCount: 300,
        saved: true
    },
    {
        _id:"42341213",
        fullName: "Steve Rogers",
        username: "steverogers04",
        title: "Stoicism. The art of not caring.😇",
        type: "article",
        article: "When we keep externals in the proper perspective, we gain a steadiness (eustatheia) that helps us along life’s way.\n\nIn the real world, we all have things we need and duties and obligations that arise from our family, relationships, and vocations. Zeno was the first to divide these externals into what he called “preferred and dispreferred indifferents.” \n\nWhat he meant is that while they don’t have intrinsic moral value, they form a kind of second class of value that is an important part of our lives—things like health and wealth are to be preferred over sickness and poverty, and if we are lucky enough to have them they can be a benefit to us and others as we pursue a virtuous life.\n😊😊😊",
        tags: ["#uplifting", "#stoic"],
        createdAt: new Date(2024, 10, 23, 16, 47, 22),
        avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-wby69l6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=61341c3ce447f8356da3146c1903395fc43d28dc",
        liked: false,
        likesCount: 200,
        commentsCount: 300,
        saved: false
    },
    // {
    //     _id: "12342",
    //     fullName: "Kal Wardin",
    //     username: "kalwardin07",
    //     title: "Elephant is my spirit animal. 🐘🐘🐘",
    //     type: "video",
    //     description: "",
    //     mediafiles: [
    //         "https://res.cloudinary.com/dv7acrjow/video/upload/v1714737320/samples/elephants.mp4",
    //         "https://res.cloudinary.com/dv7acrjow/video/upload/f_auto:video,q_auto/v1/samples/elephants"
    //     ],
    //     tags: ["#wildlife", "#elephants"],
    //     createdAt: new Date(2024, 11, 20, 16, 47, 22),
    //     avatar: "https://preview.redd.it/trying-to-come-up-with-a-new-avatar-for-my-various-social-v0-8fs49e6e1lsb1.jpg?width=519&format=pjpg&auto=webp&s=220d8e08781d7078c64e3ffc25382a18a87d5c98",
    //     liked: true,
    //     likesCount: 1500,
    //     commentsCount: 500,
    //     saved: true
    // },
]

export const sampleComments = [
    {
        _id: "123342",
        fullName: "Andromeda Galaxy",
        username: "andromeda330",
        avatar:"https://img.freepik.com/free-photo/view-3d-cool-modern-bird_23-2150946449.jpg?semt=ais_hybrid",
        text: "This is one of the most beautiful things I've ever seen. \nEYECANDY 🍬🍭",
        liked: true,
        likesCount: 200,
        commentsCount: 300,
        replies:0
    },
    {
        _id:"42213",
        fullName: "Orion Belt",
        username: "orionbelt230",
        avatar:"https://img.freepik.com/premium-photo/astronaut-outer-space-elements-this-image-furnished-by-nasa_856795-6178.jpg?w=740",
        text: "We all make mistakes in the heat of passion 😁👍. What you're saying is correct.",
        liked: false,
        likesCount: 200,
        commentsCount: 300,
        replies:3
    }
]

export const sampleChatList = [
    {
        _id: "jhalskdjashdalskdjh",
        name: "Wal Kardin",
        isGroupChat: false,
        lastMessage: "Just finished my project last night. Going to relax today!",
        participants: [
            "asdasdaskjdlaskdajlsdkasda",
            "679608f420caca28e9cd1366",
        ],
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
        updatedAt: new Date(2024, 10, 29, 16, 47, 22)
    },
    {
        _id: "jhalskdjalskdjh",
        name: "Society Members",
        isGroupChat: true,
        lastMessage: "Hello what's going on",
        participants: [
            "asdasdaskjdlaskdajlsdkasda",
            "asdasdaskjdlaskdajlsda",
            "asdasdaskjdlaskdaa",
            "679608f420caca28e9cd1366",
        ],
        admin: "679608f420caca28e9cd1366",
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
        updatedAt: new Date(2024, 10, 29, 16, 47, 22)
    },
]

export const sampelMessages = [
    {
        _id: "1",
        chat: "jhalskdjalskdjh",
        sender: "679608f420caca28e9cd1366",
        fullname: "John Corrasco",
        username: "myusername",
        content: "Hello there this is a sample message please don't consider it real",
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
    },
    {
        _id: "2",
        chat: "jhalskdjalskdjh",
        sender: "asdasdaskjdlaskdajlsdkasda",
        fullname: "Random Name",
        username: "redkoala123",
        content: "what are you saying man? This is a sample message ? Are you crazy?",
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
    },  
    {
        _id: "3",
        chat: "myusername",
        sender: "679608f420caca28e9cd1366",
        fullname: "John Corrasco",
        content: "Hello there this is a sample message please don't consider it real",
        createdAt: new Date(2024, 10, 29, 16, 47, 22),
    },
]